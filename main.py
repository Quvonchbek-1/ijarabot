...
def validate_config():
    """Fail fast with a clear message instead of silently not posting."""
    problems = []
    if not BOT_TOKEN:
...
def check_bot_access():
    """Verify the bot can actually see the target chat before doing any work."""
    try:
        res = session.get(
...
def send_to_telegram(caption, photo_url, keyboard, retries=2):
    """Send with HTML-escaped, retry-capable delivery. Returns True/False."""
    for attempt in range(1, retries + 1):
        try:
            ...
            # Photo-specific failure (e.g. broken/expired image URL) -> fall back to text-only.
...
def main():
    ...
    for item in offers:
        ...
        # Escape everything that came from OLX before wrapping our own HTML tags around it.
        ...
        if ok:
            ...
            log.error("❌ '%s' yuborilmadi, seen_ids ga qo'shilmadi (keyingi ishga tushirishda qayta urinadi).", title[:30])
